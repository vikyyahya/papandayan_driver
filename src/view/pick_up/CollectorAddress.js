...
class CollectorAddress extends Component {
  constructor(props) {
    super(props);
    this.state = {
      date: "",
  ...
  goForm() {
    this.props.navigation.navigate("FormCollectorAddress");
  }
  ...
  async actionCheck(data, index) {
    ...
    await saveData("COLLECTOR_ADDRESS", this.state.selected_address);
    // this.goBack();
  }
  ...
  async deleteAddress(item) {
    ...
    await deleteData(
      BASE_URL + DEPTOR + "/" + this.state.idSelectedAddress,
      this.state.token
    ).then((response) => {
  ...
  actionEdit(data) {
    this.props.navigation.navigate("EditFormCollectorAddress", { data: data });
  }
    ...
    return (
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.text_header}>Alamat Penagihan</Text>
        </View>
